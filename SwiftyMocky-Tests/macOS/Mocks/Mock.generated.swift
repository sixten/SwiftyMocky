...
// sourcery:end
// MARK: - ProtocolWithClosureAttributes

open class ProtocolWithClosureAttributesMock: ProtocolWithClosureAttributes, Mock {
    public init(sequencing sequencingPolicy: SequencingPolicy = .lastWrittenResolvedFirst, stubbing stubbingPolicy: StubbingPolicy = .wrap, file: StaticString = #file, line: UInt = #line) {
        SwiftyMockyTestObserver.setup()
        self.sequencingPolicy = sequencingPolicy
        self.stubbingPolicy = stubbingPolicy
    ...
    open func open(_ url: URL, completionHandler completion: (@MainActor @Sendable (Bool) -> Void)?) {
        addInvocation(.m_open__urlcompletionHandler_completion(Parameter<URL>.value(`url`), Parameter<(@MainActor @Sendable (Bool) -> Void)?>.value(`completion`)))
		let perform = methodPerformValue(.m_open__urlcompletionHandler_completion(Parameter<URL>.value(`url`), Parameter<(@MainActor @Sendable (Bool) -> Void)?>.value(`completion`))) as? (URL, (@MainActor @Sendable (Bool) -> Void)?) -> Void
		perform?(`url`, `completion`)
    }


    fileprivate enum MethodType {
        case m_open__urlcompletionHandler_completion(Parameter<URL>, Parameter<(@MainActor @Sendable (Bool) -> Void)?>)

        static func compareParameters(lhs: MethodType, rhs: MethodType, matcher: Matcher) -> Matcher.ComparisonResult {
            switch (lhs, rhs) {
            case (.m_open__urlcompletionHandler_completion(let lhsUrl, let lhsCompletion), .m_open__urlcompletionHandler_completion(let rhsUrl, let rhsCompletion)):
				var results: [Matcher.ParameterComparisonResult] = []
				...
				results.append(Matcher.ParameterComparisonResult(Parameter.compare(lhs: lhsCompletion, rhs: rhsCompletion, with: matcher), lhsCompletion, rhsCompletion, "completionHandler completion"))
				return Matcher.ComparisonResult(results)
            }
        }
        ...
        func intValue() -> Int {
            switch self {
            case let .m_open__urlcompletionHandler_completion(p0, p1): return p0.intValue + p1.intValue
            }
        }
        func assertionName() -> String {
            switch self {
            case .m_open__urlcompletionHandler_completion: return ".open(_:completionHandler:)"
            }
        }
    }
    ...
    public struct Verify {
        ...
        public static func open(_ url: Parameter<URL>, completionHandler completion: Parameter<(@MainActor @Sendable (Bool) -> Void)?>) -> Verify { return Verify(method: .m_open__urlcompletionHandler_completion(`url`, `completion`))}
    }
